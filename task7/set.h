...
class SetAbstract {
 public:
  ...
  virtual std::unique_ptr<SetAbstract> get_union(
      const SetAbstract& other) const = 0;
  virtual std::unique_ptr<SetAbstract> get_intersection(
      const SetAbstract& other) const = 0;
...
class SetList : public SetAbstract<T> {
 ...
 public:
  ...
  std::unique_ptr<SetAbstract<T>> get_union(
      const SetAbstract<T>& other) const override {
    auto result = clone();
  ...
  std::unique_ptr<SetAbstract<T>> get_intersection(
      const SetAbstract<T>& other) const override {
